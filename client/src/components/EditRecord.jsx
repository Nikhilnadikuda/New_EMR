...
const EditRecord = () => {
    ...
    useEffect(() => {
        const fetchRecord = async () => {
            await axios.get(`http://localhost:9090/medical-records/${id}`).then((response) => {
                setValid(true);
    ...
    const handlesubmit = async (e) => {
        ...
        await axios.put(`http://localhost:9090/medical-records`, updatedRecord).then(() => {
            Swal.fire({
                icon: 'success',
    ...
    const validate = async () => {
        ...
        if(patientId.length!==0 && doctorId.length!==0){
            await axios.get(`http://localhost:9090/patient/${parseInt(patientId)}`).then((response) => {
                if (!response.data) {
                    a=false;
