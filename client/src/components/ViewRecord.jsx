...
const ViewRecord = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            await axios.get(`http://localhost:9090/patient/${record.patientId}`).then((response) => {
                setPatient(response.data);
            });
            await axios.get(`http://localhost:9090/staff/doctor/${record.doctorId}`).then((response) => {
    ...
    useEffect(() => {
        const fetchRecord = async () => {
            try {
                const response = await axios.get(`http://localhost:9090/medical-records/${id}`);
